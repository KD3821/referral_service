...
def referralcode_generator(size=6, chars=string.ascii_uppercase + string.digits):
    """Генератор инвайт-кодов для последующего закрепления за новым пользователем"""
...
def passcode_generator(size=4, chars=string.digits):
    """Генератор 4-х значных кодов для входа в систему"""
...
class User(AbstractBaseUser):
    ...
    is_inviter = BooleanField(default=False)
...
class TemporaryUser(models.Model):
    """Модель для временного сохранения пользователя до завершения первой авторизации"""
    phone = DecimalField(max_digits=11, decimal_places=0, unique=True)
